...
python train.py --epochs $EPOCHS --batch_size $BATCH --ncls $NCLS     | tee results/logs/train.log

echo ">>> Step 4: 可解释性分析 (Task 4)"
python explain.py --model $MODEL_PATH --method gradcam     | tee results/logs/explain_gradcam.log
python explain.py --model $MODEL_PATH --method shap     | tee results/logs/explain_shap.log
